...
下图展示了 Raft 中快照的基本概念。每个服务器独立地进行快照操作，仅涵盖其日志中已提交的条目。大部分工作包括状态机将其当前状态写入快照。Raft 在快照中还包含少量元数据：最后被包含的索引是快照所替代的日志中的最后一个条目的索引（即状态机已应用的最后一个条目），最后被包含的任期是这个条目的任期。这些被保留下来是为了支持对快照之后的第一个日志条目的 AppendEntries 一致性检查，因为那个条目需要前一个日志的索引和任期。为了实现集群成员变更（第 6 节），快照还包括截至最后被包含索引时日志中的最新配置。一旦服务器完成写入快照，它可以删除所有直到最后被包含索引的日志条目，以及任何先前的快照。
