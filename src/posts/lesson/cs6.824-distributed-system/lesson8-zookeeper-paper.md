...
**无惊群效应的简单锁**：我们定义一个锁节点 l 来实现这样的锁。直观地说，我们将所有请求锁的客户端排成一列，每个客户端按照请求到达的顺序获得锁。因此，希望获得锁的客户端执行以下操作：

```shell
Lock
1 n = create(l + "/lock-", EPTHEMERAL | SEQUENTIAL)
...
在 "Lock" 的第 1 行中使用 "SEQUENTIAL" 标志可以使客户端获取锁的尝试与所有其他尝试按照顺序排列。如果在第 3 行中客户端的节点拥有最低的序列号，那么该客户端持有锁。否则，客户端等待那个拥有锁客户端删除该znode。

通过只观察在客户端节点之前的那个节点，我们避免了羊群效应，因为当一个锁被释放或者一个锁请求被放弃时，只会唤醒一个进程。一旦客户端正在观察的节点消失，客户端必须检查它现在是否持有锁。（之前的锁请求可能已经被放弃，并且可能有一个序列号更低的节点仍在等待或持有锁）。

释放锁就像删除代表锁请求的节点 n 一样简单。通过在创建时使用临时（EPHEMERAL）标志，崩溃的进程将自动清理任何锁请求或释放它们可能持有的任何锁。

如果该进程在持有锁期间崩溃，ZooKeeper 会自动检测到与该进程的连接断开，并迅速删除这个临时节点。这就意味着锁被自动释放，不会出现因为进程意外崩溃而导致锁一直被占用的情况。
