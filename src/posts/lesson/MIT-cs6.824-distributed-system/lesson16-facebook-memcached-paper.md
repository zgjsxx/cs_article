...
将 Memcache 用作通用缓存层时，尽管各工作负载的访问模式、内存占用以及服务质量要求各不相同，但仍需要它们共享基础设施。不同应用程序的工作负载可能会产生负面干扰，从而导致命中率下降。

为适应这些差异，我们将集群中的 Memcached 服务器划分到不同的池中。我们指定一个池（名为通配符池）作为默认池，并为那些在通配符池中存在问题的键设置单独的池。例如，我们可能会为那些频繁访问但缓存未命中代价不高的键设置一个小型池。我们也可能会为那些访问频率低但缓存未命中代价极高的键设置一个大型池。

图 5 展示了两组不同项目的工作集，一组是低变动率的，另一组是高变动率的。工作集是通过对每百万个项目中的一个项目的所有操作进行抽样来近似估算的。对于这些项目中的每一个，我们收集其最小、平均和最大项目规模。将这些规模相加并乘以一百万，以此来近似估算工作集。日工作集和周工作集之间的差异表明了变动率的大小。具有不同变动特征的项目会以一种不利的方式相互影响：仍有价值的低变动率键会在不再被访问的高变动率键之前被逐出。将这些键放置在不同的池中可以防止这种负面干扰，并使我们能够根据其缓存未命中成本来合理确定高变动率池的规模。第 7 节将提供进一步的分析。

避免不同应用程序之间相互影响，因为它们会共享memcached的基础设施，因此划分了多个pool。
