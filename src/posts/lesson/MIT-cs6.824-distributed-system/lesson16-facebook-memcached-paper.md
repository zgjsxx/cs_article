...
在这个例子中，webserver2就读取到了旧的数据。并且后续的请求在一定时间内都将获取到旧数据。

为了解决这个问题，采用了一种**远程标记（remote marker）**机制来最小化读取过时数据的概率。该机制的核心逻辑是：
...
在这种情况下，虽然当缓存未命中时可能会引入额外的延迟，但换取了减少读取过时数据的概率。

例如下面这个例子，webserver1需要更新一个数据：
- 1.webserver1在本地的memcached中设置remote marker
- 2.webserver1将数据写入主region的DB中
- 3.从memcached中删除cache
- 4.mysql数据从主region同步到其他region
- 5.删除本地memcache中的remote marker

![remote marker的原理](https://github.com/zgjsxx/static-img-repo/raw/main/blog/lesson/6.824/lesson16/paper/remote-marker.png)
