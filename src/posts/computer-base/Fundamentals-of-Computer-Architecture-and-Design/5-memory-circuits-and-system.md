...
如图下图所示，典型的SRAM架构由四个不同的模块组成：SRAM核心、地址译码器、感应放大器和内部SRAM控制器。存储核心保持即时数据。感应放大器在读取过程中将单元电压放大到完整的逻辑电平。地址译码器根据N位地址生成2^N个字线（Word Lines）。最后，控制器生成在读取或写入周期中所需的自时序脉冲。
