...
SPI 是一种单主设备串行通信协议。这意味着只有一个主设备被指定来启动并执行与从设备的所有串行通信。当 SPI 主设备希望发送或请求从设备的数据时，它首先通过将相应的从设备选择（$\overline{SS}$）信号拉低至逻辑0来选择特定的从设备，然后为从设备生成时钟信号，如下图所示。一旦选择信号和时钟信号建立，主设备就可以在每个```SCK```的负边沿通过其 ```SDO``` 端口向选定的从设备发送串行数据，同时在每个 ```SCK``` 的正边沿通过 ```SDI``` 端口采样从设备的数据。根据 ```SPI``` 协议，从设备能够发送和接收数据，但不能生成 ```SCK```。
