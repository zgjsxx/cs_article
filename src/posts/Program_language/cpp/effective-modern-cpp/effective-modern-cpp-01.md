...
- [Item1：模板中的类型推导](#item1模板中的类型推导)
  - [ParamType是一个指针或引用，但不是通用引用](#paramtype是一个指针或引用但不是通用引用)
  - [ParamType是一个通用引用](#paramtype是一个通用引用)
  - [ParamType既不是指针也不是引用(值传递)](#paramtype既不是指针也不是引用值传递)
  - [特别讨论：数组入参和函数入参](#特别讨论数组入参和函数入参)
  - [注意点1 ```const T```,```const T&```，```const T&&``` 中T推导为指针的场景](#注意点1-const-tconst-tconst-t-中t推导为指针的场景)
  - [总结](#总结)
...
- 如果expr是左值，T和ParamType都会被推导为左值引用。这非常不寻常，第一，这是模板类型推导中**唯一一种T被推导为引用**的情况。第二，虽然ParamType被声明为右值引用类型，但是最后推导的结果是左值引用。
...
f(x);                           //x是左值，所以T是int&(T推导出了引用)，
                                //通过引用折叠，param类型是int& && = int& 
...
param是一个完全独立于```cx```和```rx```的对象——是```cx```或```rx```的一个拷贝。具有常量性的```cx```和```rx```不可修改并不代表param也是一样。这就是为什么expr的常量性constness（或易变性volatileness)在推导param类型时会被忽略：因为expr不可修改并不意味着它的拷贝也不能被修改。
...
因为数组形参会视作指针形参，所以传值给模板的一个数组类型会被推导为一个指针类型。这意味着在模板函数f的调用中，它的类型形参T会被推导为```const char*```：
