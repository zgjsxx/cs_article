...
```add```指令的操作码根据所使用的寄存器以及是否存在立即数操作数而变化。对于两个64位寄存器，它们都不在r8-r15，操作码是```0x01```
...
值得一提的是，根据Intel的[add指令集](https://www.felixcloutier.com/x86/add)， 将2个64位寄存器相加有两种操作码```0x1```和```0x3```支持
...
操作码```0x1```和```0x3```都可以用于64位register的add，区别在于源寄存器和目标寄存器在```ModR/M```中存放的顺序不一样。

使用```0x1```，则源寄存器存储在ModR/M的register字段中，目标寄存器存储在ModR/M的R/M字段中。

使用```0x3```，则源寄存器存储在ModR/M的R/M字段中, 目标寄存器存储在ModR/M的register字段中。

因此```add rax, rbx```也可以按照下面的格式进行解析：
