...
当我们调用一个函数时，我们是函数调用者负责在需要时保存寄存器（"callee-preserved register"），还是我们调用的函数负责在需要时保存寄存器（"caller-preserved register"）？
...
```rax``` 称为累加器，并且一些指令隐式地使用它作为目的地。同样，```rbx``` 有时称为基址寄存器，```rcx``` 称为计数寄存器，```rdx``` 称为```双字累加器```。
...
这将从 rax（和 rdx！）读取，然后将除法写回 rax，将余数写回 rdx，即使它没有提及它们。
...
系统调用将通过 rax 返回错误代码，这意味着系统调用返回后 rax 的值可能不是您设置的值。类似地，两个“临时”寄存器 rcx 和 r11 被系统调用覆盖。

### 第二字节寄存器

寄存器 ```rax```、```rbx```、```rcx``` 和 ```rdx``` 是唯一允许通过 ```ah```、```bh```、```ch``` 和 ```dh``` 访问第二个字节的寄存器。然而，这些寄存器的使用有一些限制：它们不能与任何使用 REX 前缀的指令一起使用，而所有"新"64 位指令都使用 REX 前缀。

因此，任何使用 x86-64 添加的功能的指令都无法使用 *h 寄存器。此限制的示例包括：
...
与 mov 一样，32 位寄存器（eax、ebx 等）上的 xchg 会将高位双字隐式清零。
